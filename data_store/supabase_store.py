...
def fetch_ml_predictions_for_outcomes(
    *,
    ...
    endpoint = (
        ...
        "prediction_payload,feature_snapshot,"
        "ml_prediction_outcomes(horizon_trading_days,outcome_status)"
        f"&universe=eq.{universe}"
    ...
    *,
    ...
    while True:
        endpoint = (
            ...
            "momentum_state,is_breakout,is_volume_surge,is_pullback,"
            "drawdown_from_20d_high,drawdown_from_60d_high,ma20_slope_5d,"
            "ma50_slope_10d,rsi_change_5d,macd_histogram_change_5d,"
            "days_above_ma20,days_below_ma20,volume_trend_20d,"
            "volatility_regime,feature_version"
...
def upsert_similar_case_results(
    records: list[dict],
    *,
    supabase_url: str | None = None,
    supabase_key: str | None = None,
    ...
    for chunk in chunk_records(records, chunk_size):
        endpoint = (
            ...
            "on_conflict=query_ticker,query_date"
        )
        payload = json.dumps(chunk, allow_nan=False).encode("utf-8")
        request = Request(
            endpoint,
