...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build ML calibration report.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_paths = write_calibration_reports(report, output_dir=Path(args.output_dir))
    ...
    for warning in report["warnings"]:
        print(
            f"warning=ml_calibration:{warning['metric']}:"
            f"{warning['message']}"
        )
...
def write_calibration_reports(report: dict, *, output_dir: Path) -> dict[str, str]:
    output_dir.mkdir(parents=True, exist_ok=True)
    suffix = build_report_suffix(report)
    json_path = output_dir / f"ml_calibration_report_{suffix}.json"
    markdown_path = output_dir / f"ml_calibration_summary_{suffix}.md"
    json_path.write_text(
        json.dumps(report, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    markdown_path.write_text(
        build_calibration_summary_markdown(report),
        encoding="utf-8",
    )
    return {
