...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build Step 20 ML error analysis report from computed outcomes.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for finding in report["findings"]:
        print(
            f"finding=step20:{finding['source']}:{finding['target']}:"
            f"{finding['message']}"
        )
...
def write_reports(report: dict, *, output_dir: Path) -> dict[str, str]:
    ...
    json_path = output_dir / "step20_ml_error_analysis_v1.json"
    markdown_path = output_dir / "step20_ml_error_analysis_v1.md"
    json_path.write_text(
        json.dumps(report, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    markdown_path.write_text(
        build_step20_error_analysis_summary_markdown(report),
        encoding="utf-8",
    )
    return {
