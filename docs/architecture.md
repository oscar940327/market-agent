...
每日與每週固定 fixture 使用 `MARKET_AGENT_REPORT_REVIEW_MODE=semantic`。即使 deterministic review 通過，仍會由 LLM 對 query relevance、evidence consistency、risk balance、clarity、hallucination safety 與 overall quality 各給 1～5 分。所有分數至少 4 分才可通過；不合格時最多修訂 3 次。
