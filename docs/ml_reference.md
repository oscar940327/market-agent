...
- 三段 purged expanding walk-forward 時間外測試，最後一段保留為 holdout。訓練資料會依 target horizon 往前清除，避免 20 日 label 跨過測試邊界造成 leakage。
...
Step 28 使用 `333,232` 筆、`106` 檔股票資料，比較 Logistic Regression、Random Forest、Extra Trees、XGBoost、LightGBM、calibrated classifier 與 quantile interval models。

| Target | 最佳候選 | 品質 | Promotion |
| --- | --- | --- | --- |
| `up_5d` | calibrated Logistic Regression | `low` | reject |
| `up_10d` | XGBoost | `low` | reject |
| `up_20d` | calibrated Random Forest | `low` | reject |
| `large_drop_20d` | calibrated Random Forest | `medium` | pass |
| `forward_return_5d` | XGBoost | `low_to_medium` | reject |
| `forward_return_10d` | XGBoost | `low_to_medium` | reject |
| `forward_return_20d` | Random Forest | `low` | reject |
| `max_drop_20d` | XGBoost | `low_to_medium` | reject |

`large_drop_20d` 在 purged holdout 的 ROC AUC 約 `0.623`、recall 約 `86.5%`、precision 約 `32.6%`、Brier score 約 `0.202`。這代表它較適合作為偏保守的風險篩選器，仍不是精準的交易訊號。系統現在允許已通過的 target 單獨進入 shadow validation，所以它不再被其他未通過的 target 擋住；正式 Research Report 仍使用 production model，直到真實 outcomes 證明 candidate 長期較好並由使用者確認。

`up_20d` 的最佳候選在 purged holdout accuracy 約 `54.7%`，但 ROC AUC 只有約 `0.514`，接近隨機排序，也沒有穩定勝過 naive baseline。這不是調整顯示門檻就能修好的問題，因此系統會保留 `low`、`reduced_trust` 與方向不明確的解讀，並在每月重新比較 Extra Trees、calibrated tree models 等候選模型。只有 purged walk-forward、holdout、校準與市場環境穩定性都通過後，才允許它進入 shadow validation。
...
| 最小樣本數 | `50` |
| 上漲方向準確率 | 至少 `50%` |
| 上漲方向 ROC AUC | 至少 `50%`，低於此值代表排序能力比隨機還差 |
| downside underestimation rate | 不應高於 `20%` |
| mean absolute calibration error | 不應高於 `10%` |
| max calibration error | 不應高於 `20%` |

Calibration 的 mean error 會依每個 probability bucket 的樣本數加權；max error 只採計至少 `20` 筆的 bucket，避免單一小樣本 bucket 把整份報告誤判為 degraded。
