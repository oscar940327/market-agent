...
各 workflow 同時具有 `issues: write` 權限，只用來建立或更新 rule-based pipeline diagnosis Issue。自動化沒有 `contents: write` 權限，因此不能直接 push 到 `main`。
...
自動診斷與 Issue / PR 安全規則請看：

```text
docs/automation_maintenance.md
```
...
MARKET_AGENT_ORCHESTRATOR_MODEL=openai/gpt-5.4-mini
MARKET_AGENT_ORCHESTRATOR_MAX_STEPS=8
MARKET_AGENT_ORCHESTRATOR_MAX_REPLANS=1
...
MARKET_AGENT_RISK_MODEL=openai/gpt-5.4-mini
