...
MARKET_AGENT_REPORT_REVIEW_MODEL=
...
`MARKET_AGENT_REPORT_REVIEW_MODE=deterministic` 不會增加 LLM token；設為 `hybrid` 時，只有 deterministic review 未通過才會呼叫 reviewer/reviser。`MARKET_AGENT_REPORT_REVIEW_MODEL` 留空會沿用 `MARKET_AGENT_LLM_MODEL`。
